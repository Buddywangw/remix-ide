...
module.exports = class CompileTab {
  constructor(localRegistry) {
    ...
    self._deps = {
      ...
      swarmfileProvider: self._components.registry.get("fileproviders/swarm")
        .api,
    ...
    function scheduleCompilation() {
      ...
      if (self.data.compileTimeout)
        window.clearTimeout(self.data.compileTimeout);
      self.data.compileTimeout = window.setTimeout(
    ...
    self._deps.compiler.event.register(
      "compilationDuration",
      function tabHighlighting(speed) {
    ...
    self._deps.compiler.event.register("loadingCompiler", function start() {
      ...
      self._view.warnCompilationSlow.style.visibility = "hidden";
      self._view.compileIcon.setAttribute(
        "title",
    ...
    self._deps.compiler.event.register("compilationFinished", function finish(
      success,
      data,
      source
    ) {
      ...
      if (success) {
        ...
        self._deps.compiler.visitContracts(contract => {
          self.data.contractsDetails[contract.name] = parseContracts(
            contract.name,
            contract.object,
            self._deps.compiler.getSource(contract.file)
          );
      ...
      if (success) document.querySelector(".compileView").style.color = "";
      // @TODO: compileView tab
      else
        document.querySelector(".compileView").style.color = styles.colors.red; // @TODO: compileView tab
      ...
      if (!error && data.contracts) {
        self._deps.compiler.visitContracts(contract => {
          self._deps.renderer.error(contract.name, self._view.errorContainer, {
            type: "success"
          });
        });
      }
  ...
  render() {
    ...
    self._deps.compiler.event.register("compilerLoaded", version =>
      self.setVersionText(version)
    );
    ...
    self._view.versionSelector = yo`
      <select onchange=${onchangeLoadVersion} class="${
      css.select
    }" id="versionSelector" disabled>
        <option disabled selected>Select new compiler version</option>
      </select>`;
    if (self.data.allversions && self.data.selectedVersion)
      self._updateVersionSelector();
    self._view.version = yo`<span id="version"></span>`;

    self._view.warnCompilationSlow = yo`<i title="Compilation Slow" style="visibility:hidden" class="${
      css.warnCompilationSlow
    } fa fa-exclamation-triangle" aria-hidden="true"></i>`;
    self._view.compileIcon = yo`<i class="fa fa-refresh ${
      css.icon
    }" aria-hidden="true"></i>`;
    self._view.compileButton = yo`<div class="${
      css.compileButton
    }" onclick=${compile} id="compile" title="Compile source code">${
      self._view.compileIcon
    } Start to compile (Ctrl-S)</div>`;
    self._view.autoCompile = yo`<input class="${
      css.autocompile
    }" onchange=${updateAutoCompile} id="autoCompile" type="checkbox" title="Auto compile">`;
    self._view.hideWarningsBox = yo`<input class="${
      css.autocompile
    }" onchange=${hideWarnings} id="hideWarningsBox" type="checkbox" title="Hide warnings">`;
    if (self.data.autoCompile)
      self._view.autoCompile.setAttribute("checked", "");
    if (self.data.hideWarnings)
      self._view.hideWarningsBox.setAttribute("checked", "");
    self._view.compileContainer = yo`
      <div class="${css.compileContainer}">
        <div class="${css.info}">
          <span>Current version:</span> Penta solidity script
          <div class="${css.crow}">
    ...
    self._view.contractNames = yo`<select class="${
      css.contractNames
    }" disabled></select>`;
    self._view.contractEl = yo`
      <div class="${css.container}">
        <div class="${css.contractContainer}">
          ...
          <div title="Publish on Swarm" class="${
            css.publish
          }" onclick=${publish}>
            <i class="${
              css.copyIcon
            } fa fa-upload" aria-hidden="true"></i><span>Swarm</span>
          </div>
        </div>
        <div class="${css.contractHelperButtons}">
          <div title="Display Contract Details" class="${
            css.details
          }" onclick=${details}>Details</div>
          <div title="Copy ABI to clipboard" class="${
            css.copyButton
          }" onclick=${copyABI}>
            <i class="${
              css.copyIcon
            } fa fa-clipboard" aria-hidden="true"></i> ABI
          </div>
          <div title="Copy Bytecode to clipboard" class="${css.copyButton} ${
      css.bytecodeButton
    }" onclick=${copyBytecode}>
            <i class="${
              css.copyIcon
            } fa fa-clipboard" aria-hidden="true"></i> Bytecode
          </div>
        </div>
      </div>`;
    self._view.el = yo`
    ...
    function updateAutoCompile(event) {
      self._deps.config.set("autoCompile", self._view.autoCompile.checked);
    }
    ...
    function details() {
      ...
      if (select.children.length > 0 && select.selectedIndex >= 0) {
        ...
        Object.keys(contractProperties).map(propertyName => {
          const copyDetails = yo`<span class="${
            css.copyDetails
          }">${copyToClipboard(() => contractProperties[propertyName])}</span>`;
          const questionMark = yo`<span class="${css.questionMark}"><i title="${
            help[propertyName]
          }" class="fa fa-question-circle" aria-hidden="true"></i></span>`;
          log.appendChild(yo`<div class=${css.log}>
            <div class="${
              css.key
            }">${propertyName} ${copyDetails} ${questionMark}</div>
    ...
    function insertValue(details, propertyName) {
      ...
      if (
        ...
        if (details[propertyName] !== "") {
          try {
            node = yo`<div>${treeView.render(
              typeof details[propertyName] === "object"
                ? details[propertyName]
                : JSON.parse(details[propertyName])
            )}</div>`; // catch in case the parsing fails.
          } catch (e) {
            node = yo`<div>Unable to display "${propertyName}": ${
              e.message
            }</div>`;
          }
        } else {
    ...
    function publish() {
      ...
      if (
        selectContractNames.children.length > 0 &&
        selectContractNames.selectedIndex >= 0
      ) {
        var contract =
          self.data.contractsDetails[
            selectContractNames.children[selectContractNames.selectedIndex]
              .innerHTML
          ];
            ...
            function(err, uploaded) {
              ...
              ${err}</span>`);
                ...
                var result = yo`<div>${uploaded.map(value => {
                  return yo`<div><b>${value.filename}</b> : <pre>${
                    value.output.url
                  }</pre></div>`;
                })}</div>`;
                modalDialogCustom.alert(
                  yo`<span>Metadata published successfully.<br> <pre>${result}</pre> </span>`
                );
              }
            },
            function(item) {
  ...
  _updateVersionSelector() {
    ...
    self._view.versionSelector.appendChild(
      yo`<option disabled selected>Select new compiler version</option>`
    );
    ...
    console.log("self.data.selectedVersio", self.data.selectedVersion);
    var url;
    if (self.data.selectedVersion === "builtin") {
      ...
      if (
        self.data.selectedVersion.indexOf("soljson") !== 0 ||
        helper.checkSpecialChars(self.data.selectedVersion)
      ) {
        return console.log(
    ...
    // var preUrl = "http://10.0.2.56:8002/build/"; //localhost
    var preUrl = "http://10.0.2.56:8003/build/";
    url = preUrl + "soljson.js?" + new Date().getTime();
    if (
  ...
  fetchAllVersion(callback) {
    var self = this;
    console.log(
      "`${self.data.baseurl}/list.json`",
      `${self.data.baseurl}/list.json`
    );
    var allversions = [
      { path: "builtin", longVersion: "latest local version" }
    ];
    var selectedVersion = "builtin";
    ...
    //       if (self._components.queryParams.get().version)
